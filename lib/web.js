...
function webSocketConnection (mailserver) {
  return function onConnection (socket) {
    //const filterFrom = socket.request.session?.filterFrom ||  socket.request.cookies?.filterFrom || '';
    const filterFrom = socket.request.session?.filterFrom || socket.request.cookies?.filterFrom || '';
    console.log('filterFrom desde cookies:', filterFrom); 
        
    const newHandlers = function(email) {  
      if (!filterFrom || (email.from && email.from.some(from => from.address && from.address.toLowerCase().includes(filterFrom.toLowerCase())))) {  
        socket.emit('newMail', email);  
      }
    };
...
web.start = function (port, host, mailserver, user, password, basePathname, secure) {
  ...
  app.use(function(req, res, next) {  
    // Si es la ruta raíz y no hay un filtro establecido, mostrar la página de bienvenida  
    if (req.path === '/' && !req.cookies.filterFrom && !req.query['from']) {  
